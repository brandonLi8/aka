...
/**
 * Gets the value of a key in the given database, or returns the default value if the key does not exist.
 *
 * @param {Level} database - The database to check.
 * @param {string} key - The key to check for existence.
 * @param {any} defaultValue - The default value to return if the key does not exist.
 */
export async function getOrDefault(database, key, defaultValue) {
  try {
